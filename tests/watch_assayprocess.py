...
class UploadWorker:
    ...
    def enqueue(self, path: Path) -> None:
        if not ASSAYPROCESS_RE.match(path.name):
            return
        key = str(path)
        # Fast skip: if manifest already has this file with matching size, skip early
...
def main() -> int:
    ...
    rescan_interval = config.rescan_interval_seconds  # kept for --once mode
    ...
    # Native observer uses Windows ReadDirectoryChangesW - no initial snapshot needed
    observer = Observer()
